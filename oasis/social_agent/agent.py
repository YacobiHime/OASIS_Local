    ...
    async def perform_action_by_llm(self):
        ...
        user_msg = BaseMessage.make_user_message(
            ...
            content=(
                ...
                f"5. 【フォロー/アンフォローのルール】\n"
                f"   - 同じユーザーの投稿に複数回共感・反応した場合は、そのユーザーを follow してください。\n"
                f"   - 不快・不信感を覚えるユーザーへは unfollow（またはまだフォローしていなければスルー）してください。\n"
                f"   - follow/unfollow は他のアクション（like_post, create_comment等）と同じターンに同時に実行して構いません。\n"
                f"   - フォロー済みのユーザーの投稿は優先的に目を通してください。\n"
            ...
            [
                {
                    ...
                    "content": self.system_message.content.split("# RESPONSE METHOD")[
                        0
                    ],
                }
            ]
