...
class PlatformUtils:
    ...
    def _not_signup_error_message(agent_id):
        return {
            "success": False,
            "error": (
                f"Agent {agent_id} has not signed up and does not have " f"a user id."
            ),
        }
    ...
    def _add_comments_to_posts(self, posts_results):
        ...
        for row in posts_results:
            ...
            # Build a flat dict keyed by comment_id, then nest replies
            comment_map = {}
            for (
                c_id,
                c_post_id,
                ...
                comment_map[c_id] = {
                    ...
                    **(
                        {"score": c_num_likes - c_num_dislikes}
                        if self.show_score
                        else {
                            "num_likes": c_num_likes,
                            "num_dislikes": c_num_dislikes,
                        }
                    ),
                    "replies": [],
                }

            # Nest replies under their parent; top-level comments have no parent
            comments = []
            for c_id, comment in comment_map.items():
                parent_id = comment["parent_comment_id"]
                if parent_id is not None and parent_id in comment_map:
            ...
            if num_reports >= self.report_threshold:
                warning_message = (
                    "[Warning: This post has been reported" f" {num_reports} times]"
                )
